...
def main_worker(gpu, ngpus_per_node, args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        acc1, val_str = validate(val_loader, model, criterion, args)
        with open(f'moco_lincls_val_{epoch}.txt', 'w+') as f:
            f.write(val_str + '\n')
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        if i % args.print_freq == 0:
            entries_str = progress.display(i)
            with open(f'moco_lincls_{epoch}_{i}.txt', 'w+') as f:
                f.write(entries_str + '\n')
...
def validate(val_loader, model, criterion, args):
    ...
    return top1.avg, " * Acc@1 {top1.avg:.3f} Acc@5 {top5.avg:.3f}".format(top1=top1, top5=top5)
